...
// Create a more flexible interface for residential data
interface ResidentialProduct {
  ...
  price: string;
  viewDetailsLink?: string; // Made optional
  createdAt: string;
  updatedAt: string;
  publishedAt: string;
  imageSrc?: any[]; // Changed from imageSrc to image to match API
}

interface ApiResponse {
  data: ResidentialProduct[];
  meta: {
    pagination: {
      page: number;
      pageSize: number;
...
// Helper function to extract image URL from any Strapi format
...
// Helper function to map API data to CardData - UPDATED
const mapApiDataToCardData = (apiData: ResidentialProduct[]): CardData[] => {
  return apiData.map(product => {
    // Get the image data from the correct field (image array)
    const imageData = product.imageSrc; // Changed from product.imageSrc to product.image
    const imageUrl = extractImageUrl(imageData) || ResidentialImg1;

    // Get alt text from the first image in the array
    const firstImage = Array.isArray(imageData) && imageData.length > 0 ? imageData[0] : null;
    const altText = firstImage?.alternativeText || product.captionText;
...
const Residential = () => {
  // Updated API endpoint to residential pages
  const { loading, error, data } = useFetch<ApiResponse>('https://classic-melody-c69059d0ad.strapiapp.com/api/residentals?populate=*');

  // Fallback data in case API is not available
  const fallbackProjects: CardData[] = [
    {
      imageSrc: ResidentialImg1,
      altText: "Fallback Project",
      captionText: "Fallback Project",
      description: "This is a fallback project when API data is not available.",
      location: "Sector 104, Dwarka Expressway, Gurgaon",
      builder: "By Developer",
      size: "1500 sq.ft",
  ...
  const residentialProjects = data?.data 
  ...
  if (loading) {
    return (
      <div className="min-h-screen flex items-center justify-center">
        <div className="text-lg">Loading residential projects...</div>
      </div>
    );
  }
      ...
      <div className="h-[calc(45vh-5rem)] md:h-[calc(70vh-5rem)] m-2 rounded-4xl relative">
        ...
        <div className="relative z-10 h-full flex items-end justify-center">
          <div className="text-center text-white pb-8">
            ...
            <h1 className="font-semibold text-3xl md:text-5xl lg:text-7xl leading-tight mb-6 text-white">Residential Projects</h1>
          </div>
        </div>
      </div>
