...
import omxChk from "../assets/images/commingSoonBg.jpg";
...
// Interface for coming soon product data
  ...
  if (cards.length === 0) {
    return (
      <div className="min-h-screen w-full flex items-center justify-center">
        <div className="text-gray-500 text-lg">No upcoming projects found</div>
      </div>
    );
  }
...
// Helper function to map API data to CardData
const mapApiDataToCardData = (apiData: ComingSoonProduct[]): CardData[] => {
  return apiData.map(product => {
    // Get the image data from the correct field (image array)
    const imageData = product.imageSrc;
    const imageUrl = extractImageUrl(imageData) || omxChk; // Fallback to omxChk
...
const ComingSoon = () => {
  // You can update this API endpoint to your coming-soon collection
  const { loading, error, data } = useFetch<ApiResponse>('https://classic-melody-c69059d0ad.strapiapp.com/api/coming-soons?populate=*');
  ...
  const comingSoonProjects = data?.data 
  ...
  if (loading) {
    return (
      <div className="min-h-screen flex items-center justify-center">
        <div className="text-lg">Loading upcoming projects...</div>
      </div>
    );
  }
      ...
      <div className="h-[calc(45vh-5rem)] md:h-[calc(70vh-5rem)] m-2 rounded-4xl relative">
        ...
        <div className="relative z-10 h-full flex items-end justify-center">
          <div className="text-center text-white pb-8">
            <h1 className="font-semibold text-2xl md:text-3xl lg:text-4xl leading-tight mb-6 text-white">Upcoming Projects</h1>
            <h2 className="font-extrabold text-3xl md:text-5xl lg:text-6xl leading-tight mb-6 text-white">Coming Soon</h2>
          </div>
        </div>
      </div>
